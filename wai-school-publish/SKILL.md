...
Разрешения на команды: работай в обычном режиме разрешений Default. Если Claude Code запрашивает разрешение, коротко объясни ребёнку по-русски, что произойдёт, и попроси разрешить только это действие (например: «Сейчас появится вопрос — разреши один раз: я проверю папку перед публикацией»). Не проси ребёнка включать Auto-accept edits, Bypass permissions или --dangerously-skip-permissions.
...
2. «Нажми "Мои проекты" → "Опубликовать HTML без Claude"».
